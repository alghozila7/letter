...
class Home extends CI_Controller {
    ...
    public function disposisi($id_surat) {
        if ($this->session->userdata('logged_in') == true) {
            if ($this->session->userdata('level') == '1' OR $this->session->userdata('level') == '2') {
                ...
                $data['data_disposisi'] = $this->HomeModel->get_disposisi_keluar($id_surat);
                $this->load->view('template', $data);
            } else {
                redirect('logout');
            }
    ...
    public function tambah_pegawai() {
        if ($this->session->userdata('logged_in') == TRUE) {
            ...
            if($this->form_validation->run() == TRUE){
                $this->HomeModel->tambah_pengguna();
    ...
    public function mark($id_disposisi){
        $data = array(
            'mark' => '1'
        );
        ...
        $this->HomeModel->update_mark($where,$data,'disposisi');
        redirect('home/disposisi_masuk');
    }
    ...
    public function get_pengguna_by_id($id_pegawai) {
        if ($this->session->userdata('logged_in') == true) {
            if ($this->session->userdata('level') == '1' OR $this->session->userdata('level') == '2') {
                $data_pengguna_by_id = $this->HomeModel->get_pengguna_by_id($id_pegawai);
                echo json_encode($data_pengguna_by_id);
            } else {
                redirect('logout');
            }
        } else {
    ...
    public function laporan_surat_masuk_id(){
        $dari=$this->input->post('dari');
        $sampai=$this->input->post('sampai');
        $this->load->view('admin/laporan_surat_masuk');
    }

    public function laporan_surat_keluar_id(){
        $dari=$this->input->post('dari');
        $sampai=$this->input->post('sampai');
        $this->load->view('admin/laporan_surat_keluar');
    }
     ...
     public function pengguna() {
        if ($this->session->userdata('logged_in') == true) {
            if ($this->session->userdata('level') == '2') {
                ...
                $data['main_view'] = 'admin/pengguna';
                $data['data_pengguna'] = $this->HomeModel->get_pengguna();
    ...
    public function non_pengguna($id_pegawai){
        $data = array(
            'status' => '1'
        );
        ...
        $this->HomeModel->non_pengguna($where,$data,'pegawai');
        redirect('home/pengguna');
    }

    public function aktif_pengguna($id_pegawai){
        $data = array(
            'status' => '0'
        );
        ...
        $this->HomeModel->aktif_pengguna($where,$data,'pegawai');
        redirect('home/pengguna');
    }
